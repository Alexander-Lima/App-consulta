...
function updateCurrentItem () {
    ...
    buttons.forEach(button => {
        button.addEventListener('click', async (event) => {
            ...
            const resp = await fetch(`/app-consulta/consulta-tpi?id=${id}`, {
                method: "GET",
...
function updateStatusCounter() {
    const visibleStatus = 
        document.querySelectorAll('.cnpj[style*="display: flex;"] > li > .status > li[style*="display: inline-flex;"]');
    const total = document.querySelector('.col4');
    total.dataset.totalstatus = visibleStatus.length;
}

function markUnmarkAllFilter() {
