...
async function registerMarkUnmarkLicenseSent() {
    ...
    elements.forEach(element => {
        element.addEventListener('click', async (event) => {
            ...
            element.innerText = element.dataset.status  = isSent ? "SEM PENDÊNCIAS" : "ENVIADO";

            isSent ? element.classList.replace("license_sent", "no-pendencies") :
                element.classList.replace("no-pendencies", "license_sent");
        })
    })
}
