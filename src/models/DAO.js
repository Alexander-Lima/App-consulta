import { sanitizeCNPJ, normalizeName } from '../utilities/util.js'
...
class DAO {
    ...
    async updateOrInsertComments(cnpjId, comments) {
    ...
    async updateOrInsertccp(cnpjId, ccp) {
    ...
    async insertItem(data) {
        ...
        if(ccp || comments) {
            const id = (await this.findCnpjId(sanitizeCNPJ(cnpj)))[0]?.id
            if(comments) {
    ...
    async getUserPasswordHash(user) {
