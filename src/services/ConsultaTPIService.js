...
export default async function getTPI(dbItems) {
    return new Promise (async (res, rej) => {
        ...
        while(dbItems.length > 0) {
            const promisesResult = await getPromisesArray(dbItems, getCnpjData, maxPromises);
        ...
        while(cnpjDataArray.length > 0) {
            const promisesResult = await getPromisesArray(cnpjDataArray, getTpidata, maxPromises);
    ...
    async function getTpidata(item) {
        return new Promise(async (res) => {
            ...
            if(!error_FALHA) {
                resp = 
                await axios.post("https://sicabom.bombeiros.go.gov.br/application/server/dao_tpi.php", form)
                    .catch(err => false);
            }
