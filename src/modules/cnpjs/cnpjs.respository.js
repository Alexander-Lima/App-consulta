...
async function updateItem(data) {
    ...
    withTransaction(async (db) => {
        if(comments){
            await updateOrInsertComments(db, cnpjId, comments);
        ...
        if(ccp) {
            await updateOrInsertccp(db, cnpjId, ccp);
        } else {
...
async function updateOrInsertComments(db, cnpjId, comments) {
...
async function updateOrInsertccp(db, cnpjId, ccp) {
...
async function insertItem(data) {
    ...
    withTransaction(async (db) => {
        ...
        if(ccp || comments) {
            const id = (await findCnpjId(db, sanitizeCNPJ(cnpj)))[0]?.id;

            if(comments) {
                await updateOrInsertComments(db, id, comments);
            }
            if(ccp) {
                await updateOrInsertccp(db, id, ccp);
            }
        }
    });
}

async function findCnpjId(db, cnpj) {
